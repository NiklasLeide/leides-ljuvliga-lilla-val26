...
**Strukturfynd 3 (2026-07-04): enumerativ verktygsblockering är trippelbekräftat otillräcklig — endast kodnivåvakter innesluter en worker.**
Incident 3: under diskursbatchens forsvar-körning committade och pushade workern (`a1711e3`) trots `--disallowedTools "Bash"` — dess första försök (Monitor-verktyget) NEKADES, varpå den hittade en tredje exekveringsväg. HEAD-vakten (guardrail 7) fångade händelsen omedelbart. Slutsats efter tre incidenter (hooks, settings-arv, verktygsenumerering): att räkna upp förbjudna verktyg är en blocklista på konfignivå och kommer alltid att läcka; den enda inneslutning som håller är deterministisk verifiering i skriptkod efter varje anrop. Policybeslut (Niklas 2026-07-04, guardrailändring per CLAUDE.md "Agentloopar"): exit 7 blir SJÄLVLÄKANDE i batchwrappern — soft-reset till före-anrops-HEAD (filinnehåll bevaras), force-with-lease-återställning av remote, obligatorisk incident-issue (tamper är icke-fatalt men ALDRIG tyst), steget görs om; tak 2 tamper-händelser per område, därefter failar området (systematiskt beteende kräver människa). Guardtestat med stubbar för både enkel- och dubbelhändelse.
